...
def refresh(token_data: dict = Depends(lambda t: decode_token(t))):
    ...
    refresh_token = create_refresh_token({"sub": token_data["sub"], "role": token_data["role"]})
    return TokenResponse(access_token=access_token, refresh_token=refresh_token)
...
def get_me(current_user: User = Depends(get_current_user)):
    return UserResponse(id=str(current_user.id), email=current_user.email, full_name=current_user.full_name, role=current_user.role, is_active=current_user.is_active)
