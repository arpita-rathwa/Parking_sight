...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "boundary", geoalchemy2.Geometry("POLYGON", srid=4326), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "zone_id", UUID(as_uuid=True), sa.ForeignKey("zones.id"), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "camera_id", UUID(as_uuid=True), sa.ForeignKey("cameras.id"), nullable=False
        ),
        sa.Column("timestamp", sa.DateTime(timezone=True), nullable=False, index=True),
        sa.Column(
            "coordinates", geoalchemy2.Geometry("POINT", srid=4326), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "officer_id", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=False
        ),
        sa.Column(
            "zone_id", UUID(as_uuid=True), sa.ForeignKey("zones.id"), nullable=False
        ),
